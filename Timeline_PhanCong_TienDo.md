...
| Thành viên | Nhiệm vụ chính | Đầu ra cần bàn giao | Trạng thái | % hoàn thành | Ghi chú |
| --- | --- | --- | --- | --- | --- |
| Lê Đoàn Nhật Huy | Tích hợp `PipeManager` vào gameplay loop theo thứ tự input -> bird -> pipes -> collision -> score -> render | Game loop tổng chạy đúng thứ tự | Hoàn thành | 100% | Đã nối stage update bird, pipes, collision check và score check trong `Game::update` |
